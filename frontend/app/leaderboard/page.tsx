...
export const metadata: Metadata = {
  ...
  description: 'Everyone who has caught a duplicate on Bondsman testnet, ranked by rewards earned.',
};
    ...
    <div className="mx-auto max-w-4xl space-y-12 px-4 py-16 sm:px-6">
      <header className="max-w-3xl space-y-4">
        ...
        <p className="text-lg leading-relaxed text-muted">
          ...
          from posted bonds using the 50 / 50 split. Not a canonical on-chain
          ranking; the contract tracks agent reputation, not challenger
          reputation.
        </p>
      </header>
          ...
          {ranked.map((r, i) => (
            <li key={r.challenger}>
              <Panel className="grid grid-cols-[auto_auto_1fr_auto_auto] items-center gap-4 px-4 py-3">
                <span className="serial w-6 text-[0.62rem] text-muted">#{i + 1}</span>
                <CopyHash value={r.challenger} label={truncateHash(r.challenger)} />
                <span className="text-xs text-muted">{typeLabel(r.type)}</span>
                <span className="text-right font-mono text-sm text-bone tabular">
                  {r.successes} <span className="text-xs text-muted">slashes</span>
                </span>
