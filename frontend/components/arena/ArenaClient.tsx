...
// Codes where the whole Arena is non-functional, not just one soft error.
const RESTRICTED_CODES = new Set(['OPERATOR_AUTH_REQUIRED', 'OPERATOR_AUTH_INVALID', 'RATE_LIMITED']);

export default function ArenaClient({ heading }: { heading?: boolean }) {
  const [status, setStatus] = useState<'loading' | 'ready' | 'down'>('loading');
  const [downCode, setDownCode] = useState<string | undefined>(undefined);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof ApiError && RESTRICTED_CODES.has(err.code)) {
        setDownCode(err.code);
        setStatus('down');
        return;
      }
  ...
  if (status === 'down') return <BackendDown code={downCode} />;
