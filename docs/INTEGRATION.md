...
5. Submit the paid quote to `POST /v1/actions/submit` with `quoteHash`, `faultClass`, and delivery attestation metadata when using `delivery_contradiction`. Each paid quote can create exactly one bonded action.
