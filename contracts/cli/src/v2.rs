...
fn main() { OdraCli::new().about("Bondsman V2 deployment and operations").deploy(V2Deploy).contract::<MockCsprUSD>().contract::<BondVaultV2>().contract::<BondsmanControllerV2>().contract::<InvoicePoolV2>().contract::<DuplicateClaimVerifierV2>().contract::<DeliveryContradictionVerifierV2>().build().run(); }
